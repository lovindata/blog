...
- Follow the instructions and fill in the fields according to how the PostgreSQL container is configured in `./devops/dev-local/docker-compose.yml`
...
- 🦛🤝⚛️ **Interaction between the Tapir backend server and the React frontend**
...
The `tarp` schema is currently managed by Flyway. Our goal here is to **instruct Flyway to create our counter table during the `_.migrate` operation**. To do that, simply create the following SQL file:
...
Final part! Now onto building the Docker image. It's crucial to aim for a **swift Docker build** and the most **lightweight Docker image** possible. To pursue these goals as effectively as we can 🌈, we'll leverage the [Docker build cache](https://docs.docker.com/build/cache/) and employ [Multi-stage builds](https://docs.docker.com/build/building/multi-stage/).

- Regarding the **🗄️ Docker build cache**: It's crucial to prioritize your less frequently altered and heaviest [image layers](https://docs.docker.com/build/guide/layers/) first, typically those involving dependency installations. By doing so, the [Docker image build](https://docs.docker.com/reference/cli/docker/image/build/) process will attempt to utilize cached image layers instead of rebuilding them.
- As for the **🏗️ Multi-stage builds**: Certain image layers are temporary and exist solely for building the application, rather than for running it. Multi-stage builds offer a means to eliminate these unnecessary image layers from our final Docker image.

Here are the [`.dockerignore`](https://docs.docker.com/build/building/context/#dockerignore-files) and [`Dockerfile`](https://docs.docker.com/reference/dockerfile/) files.

```dockerignore title="./devops/prod-build/Dockerfile.dockerignore"
# Backend
/backend/**/.bloop/
/backend/.metals/
/backend/.vscode/
...
```dockerfile title="./devops/prod-build/Dockerfile"
...
!!! tip
    ...
    To build your final Docker image, ensure to utilize [docker buildx](https://docs.docker.com/reference/cli/docker/buildx/) to render your image compatible with two processor architectures - **amd64** and **arm64**. Why, you may ask? Well, according to the [Stack Oveflow Survey 2023](https://survey.stackoverflow.co/2023/#section-most-popular-technologies-operating-system), the most commonly used operating systems among developers are **Windows, MacOS, and Ubuntu**, which frequently implies the presence of **amd64** and **arm64** processor architectures.

To build and push your **multi-architecture compatible Docker image to the registry**, execute the following two commands:
...
```bash
docker buildx build -t my_registry_to_replace/tarp:0.0.0 -t my_registry_to_replace/tarp:latest --platform linux/amd64,linux/arm64 --push -f "prod-build/Dockerfile" ..
```

Here are two commands I often use for implementing and debugging the Dockerfile:

```bash
docker build -t my_registry_to_replace/tarp:local -f "prod-build/Dockerfile" ..
```

```bash
docker run -it --entrypoint /bin/sh my_registry_to_replace/tarp:local
```

After the execution is complete, if you navigate to your Docker registry, you should be able to see your images.

<figure markdown="span">
  ![Deployable Docker images](image-16.png)
